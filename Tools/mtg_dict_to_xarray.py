...
def dict_to_dataset(mtg, coordinates={}, variables={}, description=""):
    ...
    props_df["segment_index"] = props_df.index
...
'''SCRIPT'''
# mtg file from last version of Rhizodep
init = 'inputs/root00020.pckl'
output_path = 'outputs\\mtg_props.nc'

# Global MTG metadata
description = "Rhizodep root MTG properties over time (built from mtg dict properties)"

# Coordinates in topology
mtg_coordinates = dict(
    # position of root axis
    root_order=dict(unit="adim", value_example=1, description="Root classes ordering according to successive branching events"),
    # position in root axis
...
props_metadata = dict(
    segment_index=dict(unit="adim", value_example=1, description="Root segment identifier index"),
    ...
    #thermal_time_since_emergence=dict(unit="s", value_example=214200, description="not provided"),
...
mtg_props_dataset = dict_to_dataset(mtg=g, coordinates=mtg_coordinates, variables=props_metadata, description=description)

print(mtg_props_dataset)
# mtg_props_dataset.to_netcdf(path=output_path)
