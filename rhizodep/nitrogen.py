...
class DiscreteVessels:
    ...
    def __init__(self, g, Nm, influx_Nm, loading_Nm, diffusion_Nm_phloem, axial_diffusion_Nm_xylem,
                 axial_diffusion_Nm_phloem, struct_synthesis, storage_synthesis, storage_catabolism, xylem_Nm,
                 xylem_volume, phloem_Nm, phloem_volume, Nm_root_shoot_xylem, Nm_root_shoot_phloem):
    ...
    def transport_N(self, affinity_Nm_root, vmax_Nm_emergence, affinity_Nm_xylem, diffusion_phloem,
                    axial_diffusion_xylem, axial_diffusion_phloem, transport_C_regulation, transport_N_regulation,
                    xylem_to_root, phloem_to_root, epiderm_differentiation, endoderm_differentiation):
        ...
        for vid in self.vertices:
            ...
            if self.struct_mass[vid] > 0:
                ...
                neighbors = []
                self.axial_diffusion_Nm_xylem[vid] = 0
                self.axial_diffusion_Nm_phloem[vid] = 0

                for k in neighbors:
                    # Passive axial diffusion between xylem elements
                    self.axial_diffusion_Nm_xylem[vid] += (axial_diffusion_xylem * (self.xylem_Nm[k] - self.xylem_Nm[vid])
                                                * (np.pi * (xylem_to_root * (self.radius[k] + self.radius[vid]) / 2) **2))
                    self.axial_diffusion_Nm_phloem[vid] += (axial_diffusion_phloem * (self.phloem_Nm[k] - self.phloem_Nm[vid])
                                                * (np.pi * (phloem_to_root * (self.radius[k] + self.radius[vid]) / 2) ** 2))
    ...
    def update_N(self, r_Nm_struct, r_Nm_stor, xylem_to_root, phloem_to_root, time_step):
        ...
        for vid in self.vertices:
            ...
            if self.struct_mass[vid] > 0:
                ...
                xylem_Nm_content = self.xylem_Nm[vid] * self.xylem_volume[vid]
                phloem_Nm_content = self.phloem_Nm[vid] * self.phloem_volume[vid]

                self.volume[vid] = (( np.pi * self.length[vid] * (self.radius[vid]) ** 2 )
                                   * (1 - (xylem_to_root ** 2) - (phloem_to_root ** 2)))
                self.xylem_volume[vid] = (np.pi * self.length[vid]
                                         * (self.radius[vid] * xylem_to_root) ** 2)
                self.phloem_volume[vid] = (np.pi * self.length[vid]
                                          * (self.radius[vid] * phloem_to_root) ** 2)

                # Local nitrogen concentration update
                self.Nm[vid] = ((Nm_content  + time_step * (self.influx_Nm[vid]
                        + self.diffusion_Nm_phloem[vid]
                        - self.loading_Nm[vid]
                ...
                self.xylem_Nm[vid] = (xylem_Nm_content + time_step * self.loading_Nm[vid]) / self.xylem_volume[vid]
                self.phloem_Nm[vid] = (phloem_Nm_content + time_step * self.diffusion_Nm_phloem[vid]) / self.phloem_volume[vid]
                if vid == 1:
                    self.xylem_Nm[vid] -= time_step * self.Nm_root_shoot_xylem[1] / self.xylem_volume[vid]
                    self.phloem_Nm[vid] += time_step * self.Nm_root_shoot_phloem[1] / self.phloem_volume[vid]
